  ...
  run(root) {
    ...
    const variables = collectVariables(root.originalRuleset)
      .reduce((acc, variable) => {
        ...
        files[rootFilenameJson] = files[rootFilenameJson] || { set: new Set(), array: [] };
        if (!files[rootFilenameJson].set.has(name)) {
          files[rootFilenameJson].set.add(name);
          files[rootFilenameJson].array.push(name.replace(/\\/g, '/'));
        }
        return acc;
      }, {});
    ...
    Object.entries(variables).forEach(([filename, fileVariables]) => {
      ...
      const fileMerged = files[filename].array
        .reduce((obj, moduleName) => Object.assign(obj, json[moduleName]), {});

      const name = filename.split(/[/\\]/).pop()?.replace?.(/\.(module\.)?[^\.]+$/, '');
      if (name && name !== 'default') {
        this.vars[name] = fileMerged;
      }
      
      this.vars.default = Object.assign(this.vars.default, fileMerged);
    });

    fs.mkdirSync(path.dirname(this.filename), { recursive: true });

    // Write the variables to the given filename with Pretty JSON
    fs.writeFileSync(this.filename, JSON.stringify(this.vars, null, 2));
  }
}
